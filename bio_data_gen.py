...
def main(argv):
    ...
    parser.add_argument(
        "-c", "--create", help="create new adjacency matrix", action="store_true"
    )
    parser.add_argument(
        "-d", "--design", help="path to save design file to", default=""
    )
    parser.add_argument(
        "--num-core-genes",
        help="number of core genes (only with -c)",
        type=int,
        default=5,
    )
    parser.add_argument(
        "--num-genes", help="total number of genes (only with -c)", type=int, default=20
    )
    parser.add_argument(
        "-s", "--snr", help="signal to noise ratio", type=float, default=1
    )
    parser.add_argument(
        "--gamma",
    ...
    parser.add_argument("--num-cond", help="number of conditions", type=int, default=10)
    parser.add_argument("--num-rep", help="number of replicates", type=int, default=3)
...
def gen_planted_edge_data(
    ...
    if num_genes < num_genes_in_adj_mat:
        print(
            "The specified number of genes is smaller than the "
    ...
    for i in range(num_experiments):
        ...
        if true_time:
            ...
            for t in range(1, num_times + 1):
                ...
                x_rep = gen_traj(
                    num_replicates, t, adj_mat, sigma_b, sigma_c, noise_c, method
                )
                x_sample = x_rep[:, -1, :]
            ...
            x = gen_traj(
                num_replicates, num_times, adj_mat, sigma_b, sigma_c, noise_c, method
            )
        expressions.append(x)
    # Output expression file.
    sample_ids = [
        "Sample" + str(i) for i in range(num_replicates * num_experiments * num_times)
    ]
    ...
    for i in range(num_experiments):
        flattened_exp = np.concatenate(
            (
                flattened_exp,
                expressions[i].reshape(num_genes, num_times * num_replicates),
            ),
            axis=1,
        )
    df = pd.DataFrame(data=flattened_exp, columns=sample_ids, index=genes)
            ...
            for i in range(num_experiments):
                for j in range(num_times):
                    for k in range(num_replicates):
                        ...
                        f.write(
                            sample_ids[idx_sample] + "," + str(i) + "," + str(j) + "\n"
                        )
                        idx_sample += 1
    if csv_exp_file:
...
def phi_input(x_t_minus_1, adj_mat, sigma_b, sigma_c, noise_c_st, method="phi"):
    ...
    noise = np.random.normal(size=(num_replicates, num_genes)) * np.asarray(sigma_b)
    noise_c = noise_c_st.reshape(1, num_genes) * np.asarray(sigma_c)
    if method == "phi":
...
def gen_traj(
    num_replicates, num_times, adj_mat, sigma_b, sigma_c, noise_c, method="phi"
):
    ...
    for t in range(1, num_times + 1):
        x_new = phi_input(
            x[:, t - 1, :], adj_mat, sigma_b, sigma_c, noise_c[:, t - 1], method
        )
...
def gen_adj_mat(num_genes, max_in_deg, margin, rand_seed):
    ...
    for i in range(num_genes):
        ...
        regulators = np.random.choice(other_genes, size=in_degrees[i], replace=False)
