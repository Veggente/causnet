...
def main(argv):
    ...
    for opt, arg in opts:
        ...
        # Single photoperiod condition.
            ...
            with open(arg, "r") as f:
                ...
                for sub_list in cond_list:
                    cond_list_temp.append([str(element) for element in sub_list])
        ...
        #    self_reg = True
            ...
            photoperiod_set = [
                "PP" + str(idx_exp + 1) for idx_exp in np.arange(num_exp)
            ]
            strain_set = ["G1"]
            time_point_set = [
                "T" + str(idx_time + 1) for idx_time in np.arange(num_times)
            ]
        elif opt == "-E":
            ...
            if not algorithm in alg_set:
                print(
                    "Inference algorithm not recognized.  "
            ...
            for line in f:
                ...
                if not json_in_file:
                    ...
                    for idx_data in range(len(data_cell)):
                        ...
                        mean_new = (mean_extract[:, 0] + mean_extract[:, 1]) / 2
                        # Estimator of the variance of the mean.
                        var_new = (var_extract[:, 0] + var_extract[:, 1]) * (
                            num_replicates - 1
                        ) / (4 * num_replicates - 2) + (
                            mean_extract[:, 0] - mean_extract[:, 1]
                        ) ** 2 / (
                            8 * num_replicates - 4
                        )
                        mean_mat = np.concatenate(
                            (mean_mat, mean_new.reshape([mean_mat.shape[0], 1])), axis=1
                        )
                        var_mat = np.concatenate(
                            (var_mat, var_new.reshape([var_mat.shape[0], 1])), axis=1
                        )
    ...
    if json_in_file:
        ...
        if not is_perturb:
            if algorithm == "causnet":
                ...
                causnet_out = ca.ocse(
                    data_cell, num_perm, significance_level, max_in_degree
                )
            elif algorithm == "sbl":
                causnet_out = ca.sbl_grn(
                    data_cell, sparsity_threshold=sparsity_threshold, sigma_eps=epsilon
                )
            else:
            ...
            for gene, parents_for_gene in enumerate(parents):
                for idx_p, parent in enumerate(parents_for_gene):
                    ...
                    sign_dict[target_id, parent_id] = float(signs[gene][idx_p])
        else:
            edge_count = {}
            sign_count = {}
            sign_cnt_total = {}
            ...
            for idx_perturb in tqdm(range(num_perturb)):
                ...
                for idx_page, data_page in enumerate(data_cell):
                    random_matrix = np.random.randn(
                        data_page.shape[0], data_page.shape[1]
                    )
                    perturbed_data_cell.append(
                        data_page + np.sqrt(data_var[idx_page]) * random_matrix
                    )
                if algorithm == "causnet":
                    causnet_out = ca.bslr(
                        perturbed_data_cell,
                    ...
                    causnet_out = ca.ocse(
                        perturbed_data_cell, num_perm, significance_level, max_in_degree
                    )
                elif algorithm == "sbl":
                    causnet_out = ca.sbl_grn(
                        perturbed_data_cell,
                ...
                for gene, parents_for_gene in enumerate(parents):
                    for idx_parent, parent in enumerate(parents_for_gene):
                        ...
                        if (gene, parent) in sign_count:
                            sign_count[gene, parent] += signs[gene][idx_parent]
            ...
            for gene, parent in edge_count:
                ...
                if significance_level:
                    ...
                    weight[target_id, parent_id] = thickness(
                        fisher(p_values_agg[gene, parent]), 0.05
                    )
            sign_dict = {
                (gene_list.genes[u].id, gene_list.genes[v].id): float(
                    np.sign(sign_count[u, v])
                )
            ...
            for photoperiod in photoperiod_set:
                for strain in strain_set:
                    ...
                    for gene in genes:
                        ...
                        if is_perturb:
                            avg_exp_lvl_1 += np.random.normal() * np.sqrt(
                                sample_mean_var(exp_lvl_1)
                            )
                            avg_exp_lvl_2 += np.random.normal() * np.sqrt(
                                sample_mean_var(exp_lvl_2)
                            )
                            avg_exp_lvl_3 += np.random.normal() * np.sqrt(
                                sample_mean_var(exp_lvl_3)
                            )
                        f.write(
                            ",".join(
                                [
            ...
            for idx_s, strain in enumerate(strain_set):
                ...
                for idx_g, gene in enumerate(genes):
                    for idx_t, time_point in enumerate(time_point_set):
                        ...
                        data_cell[exp_count][idx_t, idx_g] = np.mean(replicates)
                        data_var[exp_count][idx_t, idx_g] = sample_mean_var(replicates)
                exp_count += 1
...
def draw_gene_network(
    labels, gene_pos, adjlist_file="", zero_based_list=[], weight_dict={}, sign_count={}
):
        ...
        if zero_based_list:
            ...
            for gene, parent in weight_dict:
                gene_network.add_edge(
                    str(parent + 1), str(gene + 1), weight=weight_dict[gene, parent]
                )
    else:
...
def generate_exp_filename(path, gene_file):
    ...
    return path + "gene-expression-level" + file_suffix(gene_file) + ".csv"
            ...
            if gene_id in gene_ids_remaining:
                ...
                for idx_sid, sid in enumerate(sample_ids):
                    ...
                    if exp_key in expression:
                        expression[exp_key].append(float(exp_str_list[idx_sid]))
                    else:
                        expression[exp_key] = [float(exp_str_list[idx_sid])]
            ...
            for pp in photoperiod_set:
                for strain in strain_set:
                    for sample_time in time_point_set:
                        ...
                        exp_sq_sum += sum([x ** 2 for x in expression[exp_key]])
...
def read_gene(gene_list_file):
    ...
    with open(gene_list_file, "r") as f:
        for line in f:
            ...
            if len(data) == 4:
                gene = Gene(data[0], data[1], float(data[2]), float(data[3]))
            else:
...
def draw_gene_network(
    ...
    for target, parent in weight:
        if weight[target, parent] >= edge_threshold:
            gene_network.add_edge(
                parent, target, weight=weight[target, parent], sign=sign[target, parent]
            )
    if gene_list.has_pos:
        gene_pos = {gene.id: (gene.pos_x, gene.pos_y) for gene in gene_list.genes}
    else:
    ...
    nx.draw_networkx_edges(
        gene_network, gene_pos, edgelist=edges, width=widths, edge_color=colors
    )
    # Draw labels.
    gene_names = {gene.id: gene.name for gene in gene_list.genes}
    nx.draw_networkx_labels(
        gene_network, gene_pos, labels=gene_names, font_size=font_size
    )
    # The widths of the arrowheads are four times those of the lines.
    if thickness == "multiplicity-ratio":
        reg_100 = mlines.Line2D(
            [], [], color="black", linewidth=width_max * 4, label="100% regulator"
        )
        reg_60 = mlines.Line2D(
            [], [], color="black", linewidth=width_max * 4 * 0.6, label="60% regulator"
        )
        reg_20 = mlines.Line2D(
            [], [], color="black", linewidth=width_max * 4 * 0.2, label="20% regulator"
        )
    elif thickness == "p-value":
        reg_100 = mlines.Line2D(
            [], [], color="black", linewidth=width_max * 4, label="p-value = 0.0"
        )
        reg_60 = mlines.Line2D(
            [], [], color="black", linewidth=width_max * 4 * 0.5, label="p-value = 0.05"
        )
        reg_20 = mlines.Line2D(
            [], [], color="black", linewidth=width_max * 4 * 0.25, label="p-value = 0.1"
        )
    else:
...
def load_dict(json_in_file):
    with open(json_in_file, "r") as f:
        ...
        dict_list = []
        for my_dict in data:
            dict_list.append({ast.literal_eval(key): my_dict[key] for key in my_dict})
    return dict_list
...
def extract_data_new(gene_list, expression_file, cond_list, parser_dict):
        ...
        for line in f:
            ...
            if gene_id in gene_ids_remaining:
                for idx_sid, sid_str in enumerate(sample_ids_str):
                    ...
                    sample_time = sid[-1]
                    other_cond = sid[:-1]
                    exp_key = (gene_id, sample_time, other_cond)
                    if exp_key in expression:
                        expression[exp_key].append(float(exp_str_list[idx_sid]))
                    else:
                        expression[exp_key] = [float(exp_str_list[idx_sid])]
    ...
    for idx_oc, other_cond in enumerate(itertools.product(*cond_list[:-1])):
        data_mean.append(np.empty((num_time_points, num_genes)))
        data_var.append(np.empty((num_time_points, num_genes)))
        for idx_g, gene in enumerate(gene_list.genes):
            for idx_t, time_point in enumerate(time_point_set):
                exp_key = (gene.id, time_point, other_cond)
                replicates = expression[exp_key]
                data_mean[idx_oc][idx_t, idx_g] = np.mean(replicates)
                data_var[idx_oc][idx_t, idx_g] = sample_mean_var(replicates)
...
def rename_genes(old_graphml_file, gene_list_file, new_graphml_file):
    ...
    for gene in gene_list.genes:
        if gene.id in gene_network.nodes():
            ...
            print("Gene {} is not found in {}.".format(gene.id, old_graphml_file))
...
def bslr_avg(
    ...
    data_cell = average_full_factorial(
        exp_df, parser_dict, num_experiments, num_times, num_genes, num_replicates
    )
    parents, signs = ca.bslr(
        data_cell, num_time_lags, max_in_degree, significance_level, self_reg, tds
    )
